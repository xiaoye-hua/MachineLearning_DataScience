...
class MLPPolicySL(MLPPolicy):
    ...
    def update(self, observations, actions):
        ...
        _, loss = self.sess.run([self.train_op, self.loss],
                                feed_dict={self.observations_pl: observations, self.acs_labels_na: actions})
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None, qvals=None):
        ...
        if self.nn_baseline:
            ...
            _, base_line_loss = self.sess.run([self.baseline_update_op, self.baseline_loss],
                                              feed_dict={self.observations_pl: observations, self.targets_n: targets_n})
        return loss
